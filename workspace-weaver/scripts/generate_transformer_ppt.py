...
# -*- coding: utf-8 -*-
"""
Transformer 架构深度解析 PPT 生成脚本
15页，面向非AI背景受众，图文并茂

基于：
- memory/transformer-ppt-content-logic.md (内容逻辑设计)
- output/transformer_diagrams.pptx (架构图页面)
- memory/transformer-ppt-plan.md (设计规范)
"""
...
# 加载架构图PPT
diag_prs = Presentation("/home/admin/.openclaw/workspace-weaver/output/transformer_diagrams.pptx")

print(f"架构图PPT有 {len(diag_prs.slides)} 页")
...
add_multiline_textbox(slide, 7.5, 1.0, 5.3, 6.0, lines,
...
# 注意力连线标注
add_textbox(slide, 0.5, 2.3, 6, 0.4, "it 对每个词的注意力权重：",
...
# 搜索框 → 标签 → 内容
add_rounded_rect(slide, 0.5, 5.0, 2.0, 0.5, "搜索关键词 (Q)", fill_color=C_ACCENT, font_size=10)
add_textbox(slide, 2.8, 5.1, 0.5, 0.3, "→", font_size=18, color=C_GRAY, align=PP_ALIGN.CENTER)
add_rounded_rect(slide, 3.3, 5.0, 2.0, 0.5, "书名标签 (K)", fill_color=C_ORANGE, font_size=10)
add_textbox(slide, 5.6, 5.1, 0.5, 0.3, "→", font_size=18, color=C_GRAY, align=PP_ALIGN.CENTER)
add_rounded_rect(slide, 6.1, 5.0, 2.0, 0.5, "书的内容 (V)", fill_color=C_GREEN, font_size=10)
...
lines = [
    "**步骤1: 生成Q、K、V**",
    "输入词向量(512维)经过三个权重矩阵变换，",
    "每个词得到64维的Q、K、V。可对所有词同时进行。",
    "",
    "**步骤2: 计算关联度打分**",
    "Q和K做点积运算，点积值越大关联度越强。",
...
# ============================================================
# 第7页：Multi-Head Attention（用架构图页面）
# ============================================================
...
# Concat + 线性投影
add_textbox(slide, 0.5, 5.0, 6, 0.4, "Concat (拼接) → 线性投影 (W^O) → 输出",
            font_size=11, bold=True, color=C_TEXT, align=PP_ALIGN.CENTER)

# 拼接框
add_rounded_rect(slide, 1.5, 5.5, 4.5, 0.5, "Concat + Linear Projection", 
                  fill_color=C_PRIMARY, font_size=11)

# 输出
add_rounded_rect(slide, 2.5, 6.2, 2.5, 0.5, "输出 (512维)", fill_color=C_GREEN, font_size=12)

# 箭头
for i in range(4):
    y = 2.3 + i * 0.65 + 0.5
    add_connector_line(slide, 0.5 + i * 1.4 + 0.6, y, 2.0, 5.5, color=head_colors[i], width=Pt(1))

add_connector_line(slide, 3.75, 6.0, 3.75, 6.2, color=C_PRIMARY, width=Pt(2))
...
# 原句
add_textbox(slide, 0.8, 1.1, 2.0, 0.3, "原句：", font_size=11, bold=True, color=C_TEXT)
for i, word in enumerate(["狗", "咬", "人"]):
    add_rounded_rect(slide, 1.5 + i * 0.8, 1.1, 0.6, 0.4, word, fill_color=C_ACCENT, font_size=11)

# 打乱句
add_textbox(slide, 0.8, 1.6, 2.0, 0.3, "打乱：", font_size=11, bold=True, color=C_TEXT)
for i, word in enumerate(["人", "咬", "狗"]):
    add_rounded_rect(slide, 1.5 + i * 0.8, 1.6, 0.6, 0.4, word, fill_color=C_ORANGE, font_size=11)
...
table_data = [
    ("方法", "原理", "优点", "缺点", "使用者"),
    ("正弦余弦", "sin/cos公式计算", "无需学习、可外推", "表达能力有限", "原始Transformer"),
    ("可学习编码", "当参数训练", "简单直观", "无法处理更长序列", "BERT、GPT-3"),
    ("RoPE旋转编码", "旋转矩阵注入Q和K", "相对位置+可外推", "计算稍复杂", "LLaMA等现代模型"),
]

for row_i, row in enumerate(table_data):
    y = 3.6 + row_i * 0.55
    col_widths = [1.2, 1.8, 1.5, 1.5, 1.5]
    x = 0.5
...
add_rounded_rect(slide, 7.3, 6.5, 2.5, 0.5, "输出: I love Beijing...", fill_color=C_GREEN, font_size=10)
...
# 标注
add_textbox(slide, 4.2, 2.0, 2.5, 0.5, "考试不能偷看后面的题\n(因果掩码)", font_size=9, color=C_ACCENT)
...
for i, (inp, tgt) in enumerate(zip(train_input, train_target)):
    ...
    add_textbox(slide, 3.3, y, 0.4, 0.35, "✓", font_size=12, color=C_GREEN, align=PP_ALIGN.CENTER)
...
# 标题
add_textbox(slide, 0.5, 0.3, 12, 0.6, "8年演进：从一篇论文到改变世界",
            font_size=22, bold=True, color=C_PRIMARY)
...
# 底部
add_textbox(slide, 2, 6.9, 9, 0.3, "感谢聆听！  |  Weaver  |  2026",
            font_size=12, color=C_GRAY, align=PP_ALIGN.CENTER)

# ============================================================
# 保存
# ============================================================
output_path = "/home/admin/.openclaw/workspace-weaver/output/Transformer_架构深度解析_v1.pptx"
